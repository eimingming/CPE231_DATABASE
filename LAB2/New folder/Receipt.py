...
class Receipt:
    ...
    def create(self, receiptNo, receiptDate, customerCode, paymentMethod, paymentReference, remark, receiptLineItemList):
        if receiptNo in self.dict:
            ...
            self.dict[receiptNo] = {"Date" : receiptDate,"Customer Code" : customerCode,"Payment Method" : paymentMethod, "Payment Reference" : paymentReference, "Total Received" : total, "Remarks" : remark,"Receipt Line Item" : receiptLineItemList}
    ...
    def update(self, receiptNo, newReceiptDate, newCustomerCode, newPaymentMethod, newPaymentReference, newRemark, newReceiptLineItemList):
        if receiptNo in self.dict:
            ...
            self.dict[receiptNo]["Receipt Line Item"] = newReceiptLineItemList
        else:
    ...
    def update_receipt_line(self, receiptNo, invoiceNo, amountPaid):
        if receiptNo in self.dict:
            receiptLineItemList = []
            bUpdated = False
            for lineItem in self.dict[receiptNo]["Receipt Line Item"]:
                invoiceLineItem = {}
            ...
            if bUpdated:
                receiptLineItemList,total = self.__updateLineItem(receiptLineItemList)
                self.dict[receiptNo]["Receipt Line Item"] = receiptLineItemList
                self.dict[receiptNo]["Total Received"] = total
            else:
