...
def report_list_all_invoices(invoices, customers, products):
    ...
    #print (result)
    printDictInCSVFormat(result, ('Invoice No',), ('Invoice Date', 'Customer Code','Due Date','Total','VAT','Amount Due','Items List'))
    return result #send result for caller program to use
...
def report_unpaid_invoices(invoices,customers,receipts):
    ...
    result = {}
    for receiptKey, receiptCol in allreceipt.items():
        for receiptLine in receiptCol['Receipt Line Item']:
            if receiptLine['Invoice No'] not in result: # ยังไม่เคยเจอ Invoice นี้เลยสร้างมาใหม่
                newValue = {}
                newValue['Invoice Amount Reveived'] = receiptLine['Amount Paid Here']
                invoice = invoices.read(receiptLine['Invoice No'])
                if invoice[0]['Is Error']:
                    newValue["Invoice Date"] = ""
                    newValue["Customer Name"] = "" 
                    newValue["Invoice Amount Due"] = ""
                    newValue["Invoice Amount Not Paid"] = ""
                else:
                    newValue["Invoice Date"] = invoice[1]['Invoice Date']
                    newValue["Invoice Amount Due"] = invoice[1]['Amount Due']
                    newValue["Invoice Amount Not Paid"] = newValue["Invoice Amount Due"] - newValue['Invoice Amount Reveived']
                    customer = customers.read(invoice[1]['Customer Code'])
                    if customer[0]['Is Error']:
                        newValue["Customer Name"] = ""
                    else:
                        newValue["Customer Name"] = customer[1]['Name']
                result[receiptLine['Invoice No']] = newValue
            else:
                result[receiptLine['Invoice No']]['Invoice Amount Reveived'] += receiptLine['Amount Paid Here']
                result[receiptLine['Invoice No']]["Invoice Amount Not Paid"] = result[receiptLine['Invoice No']]["Invoice Amount Due"] - result[receiptLine['Invoice No']]['Invoice Amount Reveived']
    printDictInCSVFormat(result, ('Invoice No',), ('Invoice Date', 'Customer Name','Invoice Amount Due','Invoice Amount Reveived','Invoice Amount Not Paid'))
    
    for line in result:
        total += result[line]["Invoice Amount Not Paid"]
...
def report_list_all_receipt(receipt, invoices, customers):
    ...
    for receiptNo, receiptDetail in allreceipt.items():
        ...
        for receiptColume, receiptData in receiptDetail.items():
            ...
            elif receiptColume == "Receipt Line Item":
                newLineItemList = []
                for lineItem in receiptDetail['Receipt Line Item']:
                    newLineItem = {}
                    for lineItemColume, lineItemData in lineItem.items():
                        if lineItemColume == "Invoice No":
                            ...
                            if invoice[0]['Is Error']:
                                newLineItem[1]["Invoice Date"] = ""
                                
                            else:
                                newLineItem["Invoice Date"] = invoice[1]["Invoice Date"]
                        else:
                            newLineItem[lineItemColume] = lineItemData
    ...
    printDictInCSVFormat(result, ('Receipt No',), ('Date', 'Customer Code','Customer Name','Payment Method','Payment Reference','Remarks','Total Received','Receipt Line Item'))
    return result #send result for caller program to use
