...
class ReceiptDetail(View):
    def get(self, request, pk, pk2):
        receipt_no = pk + "/" + pk2
        receipt = list(Receipt.objects.select_related("custome").filter(receipt_no=receipt_no).values('receipt_no', 'customer_code', 'customer_code__name', 'receipt_date', 'payment_method_code', 'payment_method_code__payment_method_name', 'payment_reference','total_receipt','remark'))
...
class ReceiptCreate(View):
    def post(self, request):
        ...
        request.POST['receipt_date'] = reFormatDateMMDDYYYY(request.POST['receipt_date'])
    ...
    def get(self, request):
        ...
        with connection.cursor() as cursor:
            cursor.execute('SELECT r.receipt_no as "Receipt No", r.receipt_date as "Receipt Date" '
                           ...
                           ' , r.payment_reference as "Payment Reference", r.total_receipt as "Total received", r.remark as "Remark" '
                           ' FROM receipt r '
                           ' LEFT JOIN customer c '
                           '    ON r.customer_code = c.customer_code '
                           ' LEFT JOIN paymentmethod pm '
                           '    ON r.payment_method_code = pm.payment_method_code '
