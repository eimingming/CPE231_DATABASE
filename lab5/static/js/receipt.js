...
$(document).ready( function () {
    ...
    $('.search_paymentmedthod_code').click(function () {
        ...
        $.ajax({
            ...
            success: function  (data) {
                ...
                data.paymentMedthods.forEach(paymentMedthod => {
                    ...
                    <tr class="d-flex">
                        <td class='col-1'>${i++}</td>
                        <td class='col-3'><a class='a_click' href='#'>${paymentMedthod.payment_method_code}</a></td>
                        <td class='col-5'>${paymentMedthod.payment_method_name}</td>
                        <td class='col-3'></td>
                        <td class='hide'></td>
                    </tr>`;
                });
        ...
        // open popup
        $('#txt_modal_param').val('payment_method_code');
        $('#modal_form').modal();
    });
    ...
    $('body').on('click', 'a.a_click', function() {
        ...
        } else if ($('#txt_modal_param').val() == 'payment_method_code') {
    ...
    $('#btnSave').click(function () {
        ...
        var payment_method_code = $('#txt_PaymentMethodName').val().trim();
...
function get_receipt_detail (receipt_no) {
    $.ajax({
        ...
        success: function  (data) {
            reset_table();
            $("#txt_PaymentMethodName").val(data.receipt.payment_method_code__payment_method_name)
            $("#txt_PaymentCode").val(data.receipt.payment_method_code)
            $("#txt_PaymentRefName").val(data.receipt.payment_reference)
