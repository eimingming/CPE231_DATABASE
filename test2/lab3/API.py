...
def report_unpaid_invoices(invoices,customers,receipts):
    ...
    return result, result2
"""
def report_everyday(invoices,customers,receipts):
    db = DBHelper()
    data, columns = db.fetch (' SELECT i.date as "Invoice Date", SUM(rli.amount_paid_here) as "Total Amount Paid Here",'
                              '(i.amount_due - sum(rli.amount_paid_here)) as "Total Amount Not Paid" '
	                            'FROM receipt r JOIN receipt_line_item rli ON r."receipt_no" = rli."receipt_no" '
	                            'JOIN invoice i ON i."invoice_no" = rli."invoice_no" '
	                            'JOIN customer c ON c."customer_code" = i."customer_code" ' 
    ...
    printDictInCSVFormat(result, ('None',), ('Invoice Date', 'Total Amount Paid Here','Total Amount Not Paid'))
